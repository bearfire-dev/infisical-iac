...
tf plan -input=false -detailed-exitcode >/dev/null || die "plan not stable after apply"
...
tf plan -input=false -detailed-exitcode >/dev/null || die "manual value caused a plan diff: provider would rewrite values"

step "6. unrelated metadata change must not touch values"
TF_VAR_metadata_marker="changed" tf apply -input=false -auto-approve >/dev/null
...
tf plan -input=false -detailed-exitcode >/dev/null || die "plan not stable after write-only import"
