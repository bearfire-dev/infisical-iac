...
| `terraform/` | Terraform root: `platform-bootstrap` Infisical project with nine placeholder credential objects (`/terraform-backend`, `/connections`), the `plan` and `apply` machine identities, their GitHub OIDC bindings, and project memberships. State: `bearfire-infisical-global-state/bootstrap/terraform.tfstate`. | first apply locally (`scripts/terraform-apply.sh bootstrap`), afterwards `.github/workflows/bootstrap.yml` (manual dispatch) |
